<template>
  <div id="IDchart">
    <div>
      ...
      <button @click="IDquery()" class="common-btn" type="button">添加</button>
    </div>
    <div class="container">
      <ul class="domain-list">
        <li v-for="item in domainList" :key="item"> {{item}} <i @click="IDremove(item)" class="el-icon-delete"></i></li>
      </ul>
...
export default {
  ...
  mounted () {
    // 重点域名表
    this.domainList = ['cn.ac.iie','com.google']
    // 画图的请求
    this.initCharts('cn.ac.iie,com.google')
    // 定时器============================todo
  },
  methods: {
    IDquery () {
    ...
    initCharts (queryNames) {
      ...
      //this.$http.post('http://172.16.8.44:8188/domain-data-analysis-system/domainHistoryRecordAnalysisController/domainHistoryRecordAnalysisQuery', {
